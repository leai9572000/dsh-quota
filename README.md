...
| 数据源 | 百分比 | 金额 |
|---|---|---|
| **官方** | 上游返回的整数百分比，与官方控制台一致 | ❌ 上游不提供，不显示 |
| **本机** | 按本机会话日志实测金额 ÷ 窗口预算计算，含一位小数 | ✅ 显示「已用 / 预算」 |

**金额只有本机模式才有**：上游接口只返回整数百分比和重置时间，实测返回体仅
`usage.<window>.{status, percent, resetsAt}`，**没有任何美元字段**
（`/usage/daily`、`/billing`、`/credits`、`/spend` 均 404），所以官方模式无从显示金额。

切到本机模式还会额外显示「今天」分组：今日花费、请求次数、token 明细与高峰时段提示。
...
- **今天**（仅本机模式）：今日花费、请求次数、token 明细、高峰时段提示。
