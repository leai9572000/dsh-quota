...
在 DeepSeek Harness Web GUI 的输入框右下角显示当前大模型的**订阅额度**与**今日花费**。

## 显示什么

胶囊（收起态）：`5 小时 6.1% · 本周 16.4% · 本月 8.2% · 今天 $1.25`
点击展开明细面板。

- **OpenCode Go 额度**：5 小时 / 本周 / 本月三个窗口，各带百分比、重置倒计时、
  「已用美元 / 预算」，以及上游整数百分比（便于与官方控制台对账）。
  数据来自 `GET https://opencode.ai/zen/go/v1/usage`（Bearer `OPENCODE_API_KEY`）。
- **今天（本机 DSH）**：今日花费（美元，两位小数）、请求次数、token 明细、高峰时段提示。
- **DeepSeek 余额**：`GET https://api.deepseek.com/user/balance`（Bearer `DEEPSEEK_API_KEY`）。
  按量付费余额，不是订阅额度；没配 key 就整块不显示。

口径只针对 **DeepSeek V4.1 Flash**（用户当前使用的模型）。

## 百分比的小数位从哪来

上游 `/zen/go/v1/usage` **只返回整数百分比**（实测响应体只有
`usage.<window>.{status, percent, resetsAt}`，`percent` 是 `6` 这种整数；
`?window=day`、`?granularity=day`、`?period=today` 全部被忽略）。

所以小数**不是**把整数补零得到的，而是用本机实测金额算出来的：
...
而上游整数只说 6%。面板同时显示两者：大号是实测小数，尾注是上游整数。

**没有本机实测值时**（新的一天、没有任何 ocgo 流量），直接显示上游整数 `6%`，
不会显示 `6.0%` —— 不给整数补零假装精度。

## 今日花费是怎么算的

上游没有按天接口，所以由 `src/spend.js` 在本机算：
...
```json
{
  "dependencies": { "dsh-quota": "file:/绝对路径/dsh-quota" },
  "dsh": { "profile": { "bundles": ["…已有 bundle…", "dsh-quota"] } }
}
```
...
- 接口返回的百分比是"整计划"还是"当前模型"未经验证（文档说限额按模型），
  换模型跑一段时间后对比即可判断。
- 本机实测窗口与上游窗口的起算口径略不同（本机 weekly 用"最近 7 天"、monthly 用"本月 1 日起"），
  所以实测小数与上游整数会有小幅出入；整数百分比始终以上游为准。
- 今日花费只覆盖本机 DSH 流量，与「本月」官方百分比（含所有客户端）不可直接相加对比。
